...
## 52. `sps_sel` 編碼重排:0→SPS8, 1→SPS16, SPS=4 移除, SPS=32 留待之後(2026-08-06)
使用者要求把 `sps_sel` 對照表改成 `0→SPS=8, 1→SPS=16`, 原本的 SPS=4 直接刪掉不留, SPS=32(原本想一起加)因為需要新係數表+可能吃掉 160MHz 時序餘裕(第 47 節的 128-tap 加總樹已經是最慢路徑), 決定先不做, `sps_sel` 維持 `ap_uint<2>` 不加寬, code 2/3 保留給以後擴充用.

- **改動**(只動 `sps_sel` 解碼/係數表選擇邏輯, 沒有動 phase accumulator/FIR MAC/LUT 這些核心運算):
  - `src/top.h`:`sps_sel` 對照表註解改成 `0→SPS=8, 1→SPS=16`, code 2/3 保留/不使用(都 fallback 到 SPS=16, 跟 code 1 一樣). `SPS_MAX`/`G_LEN_MAX`(16/128)沒變, 因為最大支援的 SPS 還是 16, 沒有新增 SPS=32.
  - `src/top.cpp`:拿掉 `g_coeff_sps4` 陣列(含 `#include "g_coeffs_sps4.inc"` 跟對應的 `ARRAY_PARTITION` pragma), `PHASE_SCALE` 從 3 筆縮成 2 筆(`{pi/8, pi/16}`, 索引順序對齊新的 `phase_idx` 編碼), `BYTE_LOOP` 裡兩處 `switch (sps_sel)`(`shift_amt`/`phase_idx` 解碼, FIR 係數表選擇)都改成 `case 0: SPS=8; default: SPS=16(涵蓋 1/2/3)`. `src/g_coeffs_sps4.inc` 檔案本身留在硬碟上沒刪(使用者只要求拿掉 SPS=4 這個支援路徑, 沒有要求刪檔案).
  - `tb/tb_top.cpp`:`TEST_SPS_SEL` 合法值從 `0/1/2` 收成 `0/1`, `SPS_TABLE` 從 `{16,8,4}` 改成 `{8,16}`, 註解更新(不再宣稱「預設值(0)是 SPS=16 且跟 g(t) 測試資料相符」, 因為 default 現在是 SPS=8).
- **驗證**(都在 `src/`/`tb/` 上直接做, 不是另開分支):
  - **`csim_design`(default, `TEST_SPS_SEL=0` → SPS=8)**:`TEST PASSED`, 512 samples(`8 bytes × 8 SPS × 8 bit = 512`, 樣本數對得上), idle carrier 起始值 `(0.999969, -6.10352e-05)` 跟第 49 節的 Q1.15 驗證數字一致.
  - **`csynth_design`**:`Target II=1, Final II=1, Depth=16`(跟第 49 節相同), `Estimated Fmax=223.99MHz`(比第 49 節的 220.51MHz 略高, 拿掉 `g_coeff_sps4` 的 mux/table 讓邏輯變輕微簡單, 合理).
  - **`csim_design`(`TEST_SPS_SEL=1` → SPS=16, 獨立跑在 repo 外的 scratch 專案, 不影響 `hls_prj/`)**:`TEST PASSED`, 1024 samples(`8 bytes × 16 SPS × 8 bit = 1024`), idle carrier起始值同樣是 `(0.999969, -6.10352e-05)`, 跟改動前(第 45/49 節, 那時候 sps_sel=0 才是 SPS=16)的數字完全一致——證實只是換了 `sps_sel` 的代碼對應, SPS=16 這個模式本身的運算結果沒有變.
- **注意, 這次沒有一併處理的事**(留給使用者決定要不要之後補):`xsim_verify/tb_xsim_top_sps16_default_coldstart.sv` 這支 testbench 的檔名跟內部註解/`localparam SPS=16`/`RUN_CYCLES` 都是基於「reset 放開後 `sps_sel` 預設值 = SPS=16」這個舊假設寫的——這次改完, reset 預設值(`sps_sel=0`)變成 SPS=8, 這支 testbench 的名字/註解已經跟實際行為對不上(雖然它還是能跑, 因為 golden 是同一次 csim_design 用同一個預設值產生的, 兩邊會自動一致, 不會跑出錯誤結果, 只是命名/敘述過時), 之後如果要重新拿它當「目前現行」的驗證基準, 建議連同檔名/註解/`RUN_CYCLES` 一起檢查更新.
- **IP 打包**:這次沒有重新跑 `export_design`, 上一版 `tfm_modulator_v1.1_20260806.zip` 的 `sps_sel` 對照表已經跟這次改動不一致(舊 zip 是 0→16/1→8/2→4), 之後要重新封裝時記得再升一次版本號.

## 53. `switch(sps_sel)` 的 `default` 語意澄清 + coldstart testbench 改名+重驗 + IP 重新封裝 v1.2(2026-08-06)
使用者對第 52 節那段 `switch (sps_sel) { case 0: ...; default: ... // SPS=16 }` 的 `default` 註解提出疑問:到底現在 reset 後真正的預設值是多少. 順便要求把 `xsim_verify/tb_xsim_top_sps16_default_coldstart.sv` 改名+更新(這支的檔名/`localparam SPS`/`RUN_CYCLES` 都還是基於「reset 預設值=SPS16」的舊假設), 並重新跑 `export_design`.

- **`default` 的語意澄清**:C++ `switch` 的 `default:` 是「沒被任何 `case` 抓到的分支」, 跟「硬體 reset 後的真正預設值」是兩個不相關的概念, 只是剛好用了同一個英文字. **`sps_sel` reset 後的真正預設值是 `0`(對應 `case 0`), 也就是 SPS=8**, 這點跟使用者的需求(第 52 節「default = sps 8」)一致, 程式邏輯本身沒有錯, 只是原本的註解容易讓人誤讀成「default 分支代表 reset 預設值」. 已把 `src/top.cpp` 兩處 `switch(sps_sel)` 的註解都改清楚, 明確標注 `case 0` 才是 `ap_rst_n` reset default.
- **testbench 改名**:`xsim_verify/tb_xsim_top_sps16_default_coldstart.sv` → `tb_xsim_top_sps8_default_coldstart.sv`(`git mv`), 同步更新 module 名稱、header 註解、`localparam SPS`(16→8)、`RUN_CYCLES`(1400→900, 對齊新的 `NUM_BYTES*SPS*8=512` 樣本數)、輸出 CSV 檔名、`$display` 訊息. `xsim_verify/README.md` 表格同步更新. 順手清掉舊檔名的孤兒產物(`output_waveform_xsim_sps16_coldstart.csv`、`wave_sps16_coldstart.wdb`, 本來就沒進 git).
- **重新跑 `scripts/run_hls.tcl`**(`echo "exit" | vitis_hls.bat -f scripts/run_hls.tcl`, 含 `csim_design`+`csynth_design`+`export_design`):`TEST PASSED`, `II=1`/`Depth=16`, `Estimated Fmax=223.99MHz`(跟第 52 節數字一致, 證實只是註解改動沒有影響行為).
